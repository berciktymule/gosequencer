...
// https://github.com/gordonklaus/portaudio/blob/master/examples/play.go
...
// Play file from disk
// It'd be nice to cache the audio data in a local map {file:data}
// in order not to read it from disk each time
func PlayFile(fileName string) {
	...
	chk(err)
	if id.String() != "FORM" {
		log.Panicln("bad file format: %s", id)
		return
	}
	_, err = data.Read(id[:])
	chk(err)
	if id.String() != "AIFF" {
		log.Panicln("bad file format")
		return
	}
	var c commonChunk
	var audio io.Reader
	for {
		...
		case "SSND":
			...
			audio = chunk
		default:
			log.Panicln("ignoring unknown chunk '%s'\n", id)
		}
	}
	...
	for remaining := int(c.NumSamples); remaining > 0; remaining -= len(out) {
		...
		case <-sig:
			defer ShutdownAudio()
			log.Fatalln("Interrupted, closing audio... ")
		default:
		}
	}
}
